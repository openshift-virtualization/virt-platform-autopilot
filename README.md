...
| File-based Swap Provisioning | Optional cheaper alternative to a dedicated swap partition; provisions file-backed swap sized to each worker's memory overcommitment, skipped on workers that have the dedicated partition. Swap activation remains managed by Swap Enablement | DP | <details><summary>opt-in</summary><code>platform.kubevirt.io/enable-file-based-swap-provisioning=true</code></details> | - |
