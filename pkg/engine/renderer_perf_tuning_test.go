...
func TestFileBasedSwapProvisioningIsOrderedBeforeActivation(t *testing.T) {
	...
	for _, unit := range units {
		...
		if !strings.Contains(contents, "RequiresMountsFor=/var/tmp") {
			t.Error("swap-provision.service must require the mount backing /var/tmp, so the swap file is never written to an unmounted directory")
		}
